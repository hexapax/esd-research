...
# I can provide the github files directly since it seems you couldn't access them.

Here's the report covering additional data sources and lines of attack beyond what's in your Phase 2 plan. The most important finding is that the **USCG Boating Accident Report Database (BARD)** has already been FOIA'd and is freely available for download right now — it contains cause-of-death fields for nearly 9,000 boating fatalities across 1995–2023 that can be filtered for electrocution. That alone could surface incidents in hours that would take weeks of manual searching.

The report identifies **9 new lines of attack (LOA7–LOA15)** on top of your existing six, organized by timeline:
...
- **PhD-track**: CDC National Death Index study (the definitive prevalence tool), medical examiner protocol survey, GIS proximity analysis

For your friend specifically, the **NDI + capture-recapture methodology** (combining BARD + NEMSIS + NEISS + news + legal records as independent sources) would make an exceptionally strong epidemiology dissertation — it's exactly the kind of multi-source prevalence estimation that the field values, and the 2025 JMIR paper on AI-powered obituary mining provides published methodological precedent she can cite.
